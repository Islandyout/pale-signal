...
- The Talari civilian GLB is an animation/scale source, not the final Talari presentation. `WorldArt._decorate_talari()` supplies the authored cranial sensory fan, asymmetric survey mantle, restrained luminous survey bars, and field transceiver; `TalariInstructor._install_authored_face_identity()` now completes the first-hour face treatment with a shallow ceramic brow mask, unequal lateral sensory nodes, split jaw rails, a throat resonator, and asymmetric temple vanes. Together these layers replace the stock-human facial read while leaving the imported animation base, NPC behavior, interaction, collision, tutorial, and save state untouched. All added geometry is generated in-project and adds no third-party license obligation.
...
- Kestra hero archaeology remains custom-layered around the audited module base. Future source assets may accelerate production, but any replacement or augmentation must preserve the substantially transformed first-hour identity rather than becoming the final presentation unchanged.
