...
func _test_archaeology_requires_distinct_evidence_passes() -> void:
	...
	a.mark_scanned()
	a.begin()
	_assert(a.required_passes == 3, "hero archaeology must preserve the authored three-stage evidence reading")
	_assert(a.evidence_layer_name() == "LOAD PATH", "first archaeology pass must read structural load-path evidence")
	var first_target := a.target_alignment
	a._advance_pass()
	_assert(a.pass_index == 1, "first archaeology lock must advance to the restraint-trace evidence pass")
	...
	_assert(a._stage_name(false) == "RESTRAINT TRACE ALIGNMENT", "player-facing archaeology stage must name the active restraint-trace layer")
	var second_target := a.target_alignment
	a._advance_pass()
	_assert(a.pass_index == 2, "second archaeology lock must advance to the alteration-sequence evidence pass")
	_assert(a.active and not a.completed, "second archaeology pass must not complete reconstruction")
	_assert(not is_equal_approx(a.target_alignment, second_target), "third archaeology pass must present a distinct chronology target")
	_assert(a.evidence_layer_name() == "ALTERATION SEQUENCE", "third archaeology pass must read alteration chronology rather than repeat alignment")
	_assert(a._stage_name(false) == "ALTERATION SEQUENCE ALIGNMENT", "player-facing archaeology stage must name the chronology evidence layer")
	a.cancel()
	a.free()
