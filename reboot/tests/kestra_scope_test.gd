...
func _init() -> void:
	...
	_assert(hud_source.contains("FIRST-HOUR EVIDENCE %d/2"), "production HUD overlay must frame progress around the two authored first-hour evidence sites")
	_assert(not hud_source.contains("SIGNAL %d/7"), "production HUD overlay must not expose the dormant seven-fragment campaign")
